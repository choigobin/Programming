...
//			s_pre = "<a href='"+url+"?page="+(idx_pre*10)+addtag+"'>"
					s_pre = "<a href='"+url+"?page="+(page-1)+addtag+"'>"
		...
		//Next 표시부분
		if(idx_start <= totpage && page <totpage) {
//			s_next="<a href='"+url+"?page="+idx_start+addtag+"'>"
					s_next="<a href='"+url+"?page="+(page+1)+addtag+"'>"
	...
	public static String pageListHan(int page, int totpage, String url, String search, String key) {
		...
		//Prev 표시부분
		if(idx_pre > 0) {
			s_pre = "<a href='"+url+"?page="+(idx_pre*10)+"&search="+search+"&key="+URLEncoder.encode(key)+"'>"
					+ "<img src=/img/i_prev.gif width=30 height=7></a>";
		}else {
			s_pre = "<img src=/img/i_prev.gif width=30 height=7>";
		}
		...
		//Next 표시부분
		if(idx_start <= totpage) {
			s_next="<a href='"+url+"?page="+idx_start+"&search="+search+"&key="+URLEncoder.encode(key)+"'>"
					+ "<img src=/img/i_next.gif width=30 height=7></a>";
		}else{
			s_next=" <img src=/img/i_next.gif width=30 height=7>";
		}
